...
import {createAccountInfo, deleteAccountInfo, deleteShippingInfo} from "../../redux/Shopping/shopping-actions";
...
export const AccountInfo = (props) => {
    ...
    const AccountInfoSchema = Yup.object().shape({
        firstName: Yup.string()
            ...
            .max(32, "Przekroczono maksymalny zakres 32 znaków").required("Pole nie może być puste"),
        lastName: Yup.string()
            .min(2, "Wprowadzona wartość jest za krótka")
            .max(32, "Przekroczono maksymalny zakres 32 znaków").required("Pole nie może być puste"),
    });
    ...
    const onContinueClick = async (values, actions) => {
        try {
            dispatch(createAccountInfo(props.user,values.firstName,values.lastName))
        } catch (error) {
    ...
    if (props.account === "create") {
        ...
        return (
            <>
                <Typography variant="h5" style={{paddingTop:"2%"}}>Twoje dane:</Typography>
                <Row>
                ...
                <Row style={{ display: "flex", padding:"1% 0 0 240px"}}>
                    <Button onClick={()=> dispatch(deleteAccountInfo(props.account.id))}><EditIcon style={{paddingRight:"10px"}}/>ZMIEŃ</Button>
                </Row>
            </>
        );
    }
